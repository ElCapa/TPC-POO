...
public class AgendaContactosTextUI
{
    ...
    public void run()  {
        ...
        do{
            ...
            /**
             * Adicionarei o comando if sobre o comando alterar
             */
            else if(command.equals("alterar")){
                try {
                    alterar();
                } catch (ContactoInexistenteException e) {
                    System.out.println(e.toString() + " Não foi possível alterar.");
                } catch (DuplicateKeyException e) {
                    System.out.println(e.toString() + " Não foi possível alterar.");
                }
            }
    ...
    /**
     * Adicionar um novo metedo alterar.
     */
    private void alterar() throws ContactoInexistenteException, DuplicateKeyException {
        System.out.print("Insira a chave do contato que deseja alterar: ");
        String key = parser.readLine();

        DetalhesContacto oldDetails = agenda.obterDetalhes(key);
        ...
        DetalhesContacto newDetails = new DetalhesContacto(newName, newPhone, newAddress);

        agenda.alterarDetalhes(key, newDetails);
